...
### [F-1] 프리미엄 웰컴 랜딩 및 인증 화면
* **기능**: 사용자의 첫 진입을 돕는 브랜드 소개 랜딩 페이지와 부드러운 화면 전환이 결합된 보안 인증 환경을 제공합니다.
* **상세**:
  * **웰컴 랜딩 인터페이스**: 최초 진입 시 무거운 폼 대신 브랜드 로고(`S.A PRO`)와 상세 플랫폼 가이드 텍스트가 담긴 고급 다크 카드 디자인 제공.
  * **입체 화면 트랜지션**: 로그인 및 회원가입 진입 시 부드러운 페이드인과 슬라이드업 효과(`.animate-fade-in-up`)가 연계되어 유려한 3D 모션을 제공.
  * **뒤로가기 유틸**: 폼 화면에서 처음 웰컴 랜딩 화면으로 복귀할 수 있는 `← 처음으로` 동선 지원.
  * **직관적인 역할 가입**: 회원가입 시 이모지 기반 그리드 아이콘(PM 👑, FE 💻, BE ⚙️ 등)을 통해 본인의 직책을 직관적으로 매핑.
  * **데모 퀵 로그인**: 개발자와 평가자의 편의를 돕는 이우진 PM 계정(woojin / 123) 퀵 로그인 버튼 지원.
...
* **상세**:
  ...
  * **검색 및 필터링 컨트롤러**: 칸반 상단에 키워드 검색 바, 우선순위 필터, 담당자 필터를 한데 묶어 실시간 카드를 선별해 내는 최적화 도구 장착.
